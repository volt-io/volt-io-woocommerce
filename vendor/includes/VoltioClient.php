...
class VoltioClient{
    ...
    function __construct(){
        ...
        add_action('wp_ajax_fetch_token', [$this, 'fetch_token']);
        add_action( 'wp_ajax_nopriv_fetch_token', [$this, 'fetch_token'] );
        add_action('wp_ajax_get_dropin_payments', [$this, 'get_dropin_payments']);
        add_action( 'wp_ajax_nopriv_get_dropin_payments', [$this, 'get_dropin_payments'] );
    }
    ...
    public function fetch_token($return = false){
        $client_id = $this->helper->get_voltio_option(['woocommerce_voltio_settings', 'client_id_' . $this->mode]);
        $client_secret = $this->helper->get_voltio_option(['woocommerce_voltio_settings', 'client_secret_' . $this->mode]);
        $username = $this->helper->get_voltio_option(['woocommerce_voltio_settings', 'api_username_' . $this->mode]);
        $password = $this->helper->get_voltio_option(['woocommerce_voltio_settings', 'api_password_' . $this->mode]);
        $grant_type = 'password';
        $data = [
            'client_id' => $client_id,
...
//        $currency = 'GBP';
        ...
        $data = [
            ...
            'payer' => [
                'reference' => $payer_ref,
                'name' => $_REQUEST['payer_name']?$_REQUEST['payer_name']:__('Anonymous', 'voltio')
            ],
        ...
        if ($error) {
            ...
            echo "Error: " . $error.', '.curl_errno($curl);
        } else {
            $response = json_decode($response, true);
